...
from .models import ComprobantesEmitidos, Contribuyente
def extrae(path):
...
def mis_comprobantes_emitidos(tabla,contribuyente):
    formato_fecha = "%d/%m/%Y"
    formato_fecha_req = "%Y-%m-%d"
    lista = []
    for renglon in tabla:
        #comprobar si la factura no está en la base de datos
        if not ComprobantesEmitidos.objects.filter(Cuit=renglon['Nro._Doc._Receptor']).filter(Punto_de_Venta=renglon['Punto_de_Venta']).filter(Numero_Desde=renglon['Número_Desde']).exists():        
            d = ComprobantesEmitidos()
            ...
            d.Cuit=renglon['Nro._Doc._Receptor']
            d.Denominacion_Receptor=renglon['Denominación_Receptor']
